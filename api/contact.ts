...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const name = (fields.name as string[] | undefined)?.[0];
  const email = (fields.email as string[] | undefined)?.[0];
  const phone = (fields.phone as string[] | undefined)?.[0];
  const position = (fields.position as string[] | undefined)?.[0];
  const province = (fields.province as string[] | undefined)?.[0];
  const experience = (fields.experience as string[] | undefined)?.[0];
  const message = (fields.message as string[] | undefined)?.[0];

  if (!name || !email) {
    return res.status(400).json({ success: false, error: "Name and email are required." });
  }

  const resumeFile = (files.resume as formidable.File[] | undefined)?.[0];
...
<body style="font-family: Arial, sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto;">
  ...
  <div style="background: #f8fafc; padding: 30px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 8px 8px;">
    ...
    <div style="margin-top: 24px; padding: 16px; background: #e8f5e9; border-radius: 6px; border-left: 4px solid #27ae60;">
      <p style="margin: 0; font-size: 13px; color: #2d6a4f;">
        <strong>Resume attached:</strong> ${resumeFile.originalFilename} (${((resumeFile.size || 0) / 1024).toFixed(1)} KB)
      </p>
    </div>` : ""}
  </div>
</body>
  ...
  try {
    await transporter.sendMail({
      ...
      html: htmlBody,
      ...(resumeFile && resumeFile.filepath ? {
        attachments: [
          {
    ...
    return res.status(500).json({ success: false, error: "Failed to send application. Please try again or email us directly." });
  }
}
